...
public class VisualUtils {
    ...
    public static List<FieLdsModel> deleteMore(List<FieLdsModel> modelList) {
        ...
        for (FieLdsModel model : modelList) {
            ...
            if (newList == null || ExtnKeyConsts.CHILD_TABLE.equals(model.getConfig().getExtnKey())) {
    ...
    public static DownloadVO createModelExcel(VisualdevEntity visualdevEntity, List<Map<String, Object>> list, Collection<String> keys, String sheetName) {
        ...
        try {
            ...
            for (Map<String, Object> map : list){
                ...
                for (String key : keys){
                    //子表
                    if (key.toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX)){
            ...
            allFields.stream().filter(a->!a.getConfig().getExtnKey().equals(ExtnKeyConsts.CHILD_TABLE)).forEach(m->mainMap.put(m.getVModel(),m.getConfig().getLabel()));
            List<FieLdsModel> childFields = allFields.stream().filter(a -> a.getConfig().getExtnKey().equals(ExtnKeyConsts.CHILD_TABLE)).collect(Collectors.toList());
            //创建导出属性对象
            ...
            FileInfo fileInfo = FileUploadUtils2.uploadFile(multipartFile, temporaryFilePath, fileName);
    ...
    public static void recursionFields(List<FieLdsModel> fieLdsModelList, List<FieLdsModel> allFields){
        for (FieLdsModel fieLdsModel : fieLdsModelList){
            if (ExtnKeyConsts.CHILD_TABLE.equals(fieLdsModel.getConfig().getExtnKey())){
...
//    }
    public static String exampleExcelMessage(FieLdsModel model){
        String message = "";
        String extnKey = model.getConfig().getExtnKey();
        switch (extnKey)
        {
