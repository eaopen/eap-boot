...
public class OnlineFilterUtil {
    ...
    public QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder solveValue(QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder where, SqlTable sqlTable) {
        ...
        MyType myType = myControl(extnKey);
        if (fieldValue == null) {
            fieldValue = "";
        }
        try {
            ...
            if (splitKey.contains(extnKey) && "between".equals(operator)) {
                List<String> data = JsonUtil.getJsonToList(fieldValue, String.class);
                fieldValue = data.get(0);
                fieldValue2 = data.get(1);
            }
            // 显示组织还是部门,全部拿最后一级
            if (extnKey.equals("currOrganize") && StringUtils.isNoneBlank(fieldValue)) {
                List<String> data = JsonUtil.getJsonToList(fieldValue, String.class);
            ...
            // 替换副表的字段
            if (field.indexOf("_extn_") > 0) {

                sqlTable = this.subSqlTableMap.get(field);
                field = field.split("_extn_")[1];
            }
    ...
    private boolean preHandle() {
        if(params!=null){
            ...
            // 如果是主表
            if(onlySubTable && !field.contains("_extn_") && !field.contains("-")){
                return false;
            }
            // 不拼接副表
            if(onlySubTable && field.contains("_extn_")){
                return false;
            }
        }
     ...
     * @return 控件类型
     */
    public MyType myControl(String extnKey) {
        MyType myType;
        switch (extnKey) {
            case "comInput":
            case "textarea":
...
//                        where.and(sqlTable.column(field), SqlBuilder.isLessThanOrEqualTo(DateUtil.daFormat(time2)));
    ...
    class SelectControl extends MyType {
        ...
        void judge(QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder where, SqlTable sqlTable, String field) {
            ...
            if (StringUtils.isNoneBlank(fieldValue) && fieldValue.charAt(0) == '[' && !selectIgnore.contains(extnKey)) {
            ...
            if (selectIgnore.contains(extnKey) && StringUtils.isBlank(fieldValue)) {
                fieldValue = "[]";
            }
            if ("&&".equals(logic)) {
                switch (operator) {
                ...
                switch (operator) {
                    ...
                    case "notLike":
                        ...
                        break;
                    case "in":
                        if (selectIgnore.contains(extnKey)) {
                            convertSqlServerLike();
                            where.or(sqlTable.column(field), SqlBuilder.isLike(fieldValue));
                        } else {
                            if (list.size() > 0) {
