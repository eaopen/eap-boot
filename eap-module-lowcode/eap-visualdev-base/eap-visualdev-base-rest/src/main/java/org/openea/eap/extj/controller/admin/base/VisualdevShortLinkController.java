...
import org.openea.eap.extj.config.ExtnOauthConfig;
...
@RestController
@RequestMapping("/visualdev/ShortLink" )
public class VisualdevShortLinkController extends SuperController<VisualdevShortLinkService, VisualdevShortLinkEntity> {
    ...
    @Autowired
    private ExtnOauthConfig oauthConfig;
    ...
    private String geturl(String id, String type) {
        String url = oauthConfig.getExtnDomain() + "/visualdev/ShortLink/trigger/" + id + "?encryption=";
...
//    @SaCheckPermission("onlineDev.webDesign" )
    public ActionResult saveOrupdate(@RequestBody VisualdevShortLinkForm data) {
        ...
        if(entity.getFormLink().contains(oauthConfig.getExtnDomain())){
            entity.setFormLink(entity.getFormLink().replace(oauthConfig.getExtnDomain(),""));
        }
        if(entity.getColumnLink().contains(oauthConfig.getExtnDomain())){
            entity.setColumnLink(entity.getColumnLink().replace(oauthConfig.getExtnDomain(),""));
        }
    ...
    public ActionResult getlink(@PathVariable("id" ) String id,
        ...
        if (entity != null) {
            if (DeviceType.PC.equals(deviceType)) {
                link = oauthConfig.getExtnFrontDomain() + entity.getRealPcLink();
            } else {
                link =  oauthConfig.getExtnAppDomain() + entity.getRealAppLink();
            }
        } else {
