    ...
    public static List<Map<String, Object>> getNoSwapList(List<Map<String, Object>> list, List<VisualColumnSearchVO> searchList) {
        ...
        for (Map<String, Object> dataVo : list) {
            ...
            for (VisualColumnSearchVO vo : searchList) {
                ...
                //多选框默认添加多选属性
                if (vo.getConfig().getExtnKey().equals(ExtnKeyConsts.CHECKBOX) || ExtnKeyConsts.CASCADER.equals(vo.getConfig().getExtnKey())) {
                    vo.setMultiple(true);
                }
                ...
                if (vo.getSearchType().equals("3")) {
                    String key = vo.getConfig().getExtnKey();
                    switch (key) {
                    ...
                    if (StringUtil.isEmpty(String.valueOf(dataMap.get(swapVModel))) || String.valueOf(dataMap.get(swapVModel)).equals("[]")
                        ...
                        switch (extnKey) {
                            ...
                            case ExtnKeyConsts.DATE:
                                ...
                                String dateSwapInfo = swapDataVo.getFormat() != null ? swapDataVo.getFormat() : swapDataVo.getType() != null && swapDataVo.getType().equals(ExtnKeyConsts.DATE) ? "yyyy-MM-dd" : "yyyy-MM-dd HH:mm:ss";
                        ...
                        //转换数据接口的数据
                        if (dataType != null) {
                            if (!extnKey.equals(ExtnKeyConsts.CASCADER) && !extnKey.equals(ExtnKeyConsts.CHECKBOX)) {
                                //静态数据
                                if (dataType.equals(OnlineDataTypeEnum.STATIC.getType())) {
                                    ...
                                    if (extnKey.equals(ExtnKeyConsts.TREESELECT)) {
                                        //树形缓存Key
                                        redisKey = String.format("%s-%s-%s", dsName, OnlineDataTypeEnum.DYNAMIC.getType(), swapDataVo.getConfig().getPropsUrl());
                                    }
...
//                        }
                ...
                if (dataType != null) {
                    ...
                    if (swapDataVo.getConfig().getExtnKey().equals(ExtnKeyConsts.CASCADER) || swapDataVo.getConfig().getExtnKey().equals(ExtnKeyConsts.TREESELECT)) {
