            ...
            for (FieLdsModel swapDataVo : swapDataVoList) {
                ...
                try {
                    ...
                    if (StringUtil.isEmpty(String.valueOf(dataMap.get(swapVModel))) || String.valueOf(dataMap.get(swapVModel)).equals("[]")
                        ...
                        if (extnKey.equals(ExtnKeyConsts.CHILD_TABLE)) {
...
//							dataCopyMap.putAll(dataMap);
                        ...
                        switch (extnKey) {
                            ...
                            case ExtnKeyConsts.TREESELECT:
                                ...
                                break;
                            case ExtnKeyConsts.RELATIONFORM:
                                //取关联表单数据 按绑定功能加字段区分数据
                                redisKey = String.format("%s-%s-%s-%s-%s", dsName, ExtnKeyConsts.RELATIONFORM, swapDataVo.getModelId(), swapDataVo.getRelationField(), dataMap.get(vModel));
                                ...
                                break;
                            case ExtnKeyConsts.POPUPSELECT:
                            ...
                            case ExtnKeyConsts.DATE:
                                ...
                                String format = swapDataVo.getFormat() != null ? swapDataVo.getFormat() : swapDataVo.getType() != null && swapDataVo.getType().equals(ExtnKeyConsts.DATE) ? "yyyy-MM-dd" : "yyyy-MM-dd HH:mm:ss";
                                if (isList && (ExtnKeyConsts.MODIFYTIME.equals(extnKey) || ExtnKeyConsts.CREATETIME.equals(extnKey))) {
                                    format = "yyyy-MM-dd HH:mm";
                                }
                                DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(format);
                                String date = dateTimeFormatter.format(dateTime);
                                dataMap.put(vModel, date);
                                if (ExtnKeyConsts.MODIFYTIME.equals(extnKey) || ExtnKeyConsts.CREATETIME.equals(extnKey)) {
                                    dataMap.put(swapDataVo.getVModel(), date);
                                }
                                break;
                            case ExtnKeyConsts.RATE:
                            ...
                            case ExtnKeyConsts.UPLOADIMG:
                                ...
                                break;
                            case ExtnKeyConsts.CHILD_TABLE:
        ...
        try {
            ...
            for (FieLdsModel swapDataVo : swapDataVoList) {
                ...
                if (dataType != null) {
                    ...
                    List<Map<String, Object>> options = new ArrayList<>();
                    if (swapDataVo.getConfig().getExtnKey().equals(ExtnKeyConsts.POPUPSELECT) || swapDataVo.getConfig().getExtnKey().equals(ExtnKeyConsts.POPUPTABLESELECT)) {
    ...
    private static void getDataAttr(List<FieLdsModel> fieLdsModelList, Map<String, Object> dataMap, Map<String, Map<String, Object>> dataDetailMap) {
        for (FieLdsModel fieLdsModel : fieLdsModelList) {
            ...
            if (extnKey.equals(ExtnKeyConsts.RELATIONFORM_ATTR) || extnKey.equals(ExtnKeyConsts.POPUPSELECT_ATTR)) {
                //展示数据 ? 存储数据
    ...
    public static void swapCodeDataInfo(List<FormModel> codeList, Map<String, Object> swapDataMap, Map<String, Object> dataMap) {
        for (FormModel formModel : codeList) {
            ...
            if (extnKey.equals(ExtnKeyConsts.QR_CODE) || extnKey.equals(ExtnKeyConsts.BARCODE)) {
    ...
    private boolean checkExcelData(List<FieLdsModel> modelList, Map<String, Object> data, Map<String, Object> localCache, Map<String, Object> insMap,
        ...
        for (FieLdsModel swapDataVo : modelList) {
            try {
                ...
                if (ExtnKeyConsts.CHECKBOX.equals(extnKey)) {
                    multiple = true;
                }
                ...
                cusCheckModel.setControlType(extnKey);
...
//						}
                    ...
                    case ExtnKeyConsts.DATE:
                        ...
                        try {
                            ...
                            //判断时间是否在设置范围内
                            boolean timeHasRangeError = FormPublicUtils.dateTimeCondition(swapDataVo, format, value, insMap, extnKey);
                            if (timeHasRangeError) {
                                errInfo.add(label + "值不在范围内");
                                break;
                            }
                ...
                if (dataType != null) {
                    ...
                    boolean isCascader = ExtnKeyConsts.CASCADER.equals(extnKey);
    ...
    public static void swapDatetime(List<FieLdsModel> list, Map<String, Object> map) {
        ...
        //子表
        for (FieLdsModel field : fields) {
            if (field.getVModel().toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX)) {
