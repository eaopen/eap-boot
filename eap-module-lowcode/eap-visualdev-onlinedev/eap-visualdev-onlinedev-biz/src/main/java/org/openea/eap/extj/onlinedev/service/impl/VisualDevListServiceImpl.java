...
public class VisualDevListServiceImpl extends SuperServiceImpl<VisualdevModelDataMapper, VisualdevModelDataEntity> implements VisualDevListService {
    ...
    public List<Map<String, Object>> getDataList(VisualDevJsonModel visualDevJsonModel, PaginationModel paginationModel) throws WorkFlowException {
        ...
        if (visualTables.size() > 0) {
            ...
            for (VisualColumnSearchVO vo : searchVOList) {
                ...
                if (vModel.toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX)) {
                ...
                } else if (vModel.contains("extn_" )) {
                    String fieldName = vModel.substring(vModel.lastIndexOf("extn_" )).replace("extn_" , "" );
                    String tableName = vModel.substring(vModel.indexOf("_" ) + 1, vModel.lastIndexOf("_extn" ));
                    vo.setTable(tableName);
    ...
    public List<Map<String, Object>> getListWithTable(VisualDevJsonModel visualDevJsonModel, PaginationModel paginationModel,
        ...
        try {
            ...
            allFieLds.stream().filter(f -> f.getConfig().getExtnKey().equals(ExtnKeyConsts.CHILD_TABLE)).forEach(f -> {
            ...
            if (StringUtil.isNotEmpty(paginationModel.getSuperQueryJson())) {
                ...
                superQueryList.stream().forEach(sup -> {
                    ...
                    String mastKey = fieLdsModel.getConfig().getExtnKey();
                    if (field.toLowerCase().contains(ExtnKeyConsts.CHILD_TABLE_PREFIX)) {
                    ...
                    if (mastKey.equals(ExtnKeyConsts.CHECKBOX) || mastKey.equals(ExtnKeyConsts.ADDRESS) || mastKey.equals(ExtnKeyConsts.CASCADER) || mastKey.equals(ExtnKeyConsts.COMSELECT)) {
            ...
            String sortField;
            if (sidx.matches(reg)) {
                sortField = sidx.substring(sidx.lastIndexOf("extn_" )).replace("extn_" , "" );
                sortTableName = sidx.substring(sidx.indexOf("_" ) + 1, sidx.lastIndexOf("_extn" ));
            } else {
            ...
            if (paginationModel.getDataType() == null) {
                SelectStatementProvider renderCount = whereCount.configureStatement(c -> c.setNonRenderingWhereClauseAllowed(true)).build().render(RenderingStrategies.MYBATIS3);
                count= flowFormDataMapper.count(renderCount);
                if(count == 0){
                    return Collections.EMPTY_LIST;
                }
                //树形和分组不需要分页。有脏数据传添加判断
                if(columnDataModel.getType()!=null && columnDataModel.getType()!=5 && columnDataModel.getType()!=3){
            ...
            if (idStringList.size() > 0) {
                //处理子表
                List<String> tableFields = collect.stream().filter(c -> c.toLowerCase().contains(ExtnKeyConsts.CHILD_TABLE_PREFIX)).collect(Collectors.toList());
                    ...
                    if (childFields != null) {
                        ...
                        for (String c : childFields) {
                            ...
                            if ((isDm||isOracle) && thisFieLdsModel != null) {
                                String extnKey = thisFieLdsModel.getConfig().getExtnKey();
