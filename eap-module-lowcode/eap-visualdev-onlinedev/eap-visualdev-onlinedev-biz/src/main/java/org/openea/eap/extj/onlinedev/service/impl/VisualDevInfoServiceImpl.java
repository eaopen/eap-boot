...
public class VisualDevInfoServiceImpl implements VisualDevInfoService {
    ...
    public VisualdevModelDataInfoVO getEditDataInfo(String id, VisualdevEntity visualdevEntity) {
        ...
        List<FormAllModel> table = formAllModel.stream().filter(t -> FormEnum.table.getMessage().equals(t.getExtnKey())).collect(Collectors.toList());
        ...
        try {
            ...
            List<String> mainTableFields = mast.stream().filter(m -> StringUtil.isNotEmpty(m.getFormColumnModel().getFieLdsModel().getVModel()))
                    .map(s ->
                            {
                                String extnKey = s.getFormColumnModel().getFieLdsModel().getConfig().getExtnKey();
            ...
            while (entryIterator.hasNext()) {
                ...
                List<BasicColumn> mastTableBasicColumn = childMastTableList.stream().filter(m -> StringUtil.isNotEmpty(m.getField()))
                        .map(m -> {
                            String extnKey = m.getMastTable().getFieLdsModel().getConfig().getExtnKey();
            ...
            table.stream().map(t -> t.getChildList()).forEach(
                    t1 -> {
                        ...
                        List<BasicColumn> childFields = t1.getChildList().stream().filter(t2 -> StringUtil.isNotEmpty(t2.getFieLdsModel().getVModel()))
                                .map(
                                        t2 -> {
                                            String extnKey = t2.getFieLdsModel().getConfig().getExtnKey();
                                            String modelFiled = t2.getFieLdsModel().getVModel();
                                            String aliasName = "";
                                            if (oracle || IS_DM) {
                                                if (extnKey.equals(ExtnKeyConsts.UPLOADFZ) || extnKey.equals(ExtnKeyConsts.UPLOADIMG)
                                                        || extnKey.equals(ExtnKeyConsts.EDITOR)) {
    ...
    public VisualdevModelDataInfoVO getDetailsDataInfo(String id, VisualdevEntity visualdevEntity) {
        ...
        List<FormAllModel> table = formAllModel.stream().filter(t -> FormEnum.table.getMessage().equals(t.getExtnKey())).collect(Collectors.toList());
        List<FormAllModel> mastTable = formAllModel.stream().filter(t -> FormEnum.mastTable.getMessage().equals(t.getExtnKey())).collect(Collectors.toList());
        List<FormModel> codeList = formAllModel.stream().filter(t -> t.getExtnKey().equals(FormEnum.BARCODE.getMessage())
                || t.getExtnKey().equals(FormEnum.QR_CODE.getMessage())).map(formModel -> formModel.getFormModel()).collect(Collectors.toList());
                    ...
                    .map(s ->
                            {
                                ...
                                if (oracle || IS_DM) {
                                    if (extnKey.equals(ExtnKeyConsts.UPLOADFZ) || extnKey.equals(ExtnKeyConsts.UPLOADIMG) || extnKey.equals(ExtnKeyConsts.EDITOR)
                                            || extnKey.equals(ExtnKeyConsts.ADDRESS)) {
                                        modelFiled = "dbms_lob.substr( " + modelFiled + ")";
                                    }
                                }
                                return modelFiled;
