...
public class VisualdevModelDataServiceImpl extends SuperServiceImpl<VisualdevModelDataMapper, VisualdevModelDataEntity> implements VisualdevModelDataService {
    ...
    public List<FormDataField> fieldList(String id, Integer filterType) {
        ...
        notInList.add(ExtnKeyConsts.RELATIONFORM);
        notInList.add(ExtnKeyConsts.CHILD_TABLE);
        if (Objects.equals(filterType,1)) {
            notInList.add("link");
            notInList.add("button");
            notInList.add("ExtnText");
            notInList.add("alert");
            notInList.add(ExtnKeyConsts.QR_CODE);
            notInList.add(ExtnKeyConsts.BARCODE);
            notInList.add(ExtnKeyConsts.BILLRULE);
            notInList.add(ExtnKeyConsts.CREATEUSER);
            notInList.add(ExtnKeyConsts.CREATETIME);
            notInList.add(ExtnKeyConsts.UPLOADIMG);
            notInList.add(ExtnKeyConsts.UPLOADFZ);
            notInList.add(ExtnKeyConsts.MODIFYUSER);
            notInList.add(ExtnKeyConsts.MODIFYTIME);
        }
        ...
        List<FormDataField> formDataFieldList = mainFieldModelList.stream().filter(fieLdsModel ->
                ...
                && !notInList.contains(fieLdsModel.getConfig().getExtnKey())
        ).map(fieLdsModel -> {
