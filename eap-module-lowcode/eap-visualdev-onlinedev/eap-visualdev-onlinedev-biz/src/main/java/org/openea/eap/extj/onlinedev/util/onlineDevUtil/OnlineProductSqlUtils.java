...
public class OnlineProductSqlUtils {
    ...
    public static void getColumnListSql(List<OnlineDynamicSqlModel> sqlModels, VisualDevJsonModel visualDevJsonModel,
        ...
        try {
            ...
            List<String> mainTableFields = columnFieldList.stream().filter(s -> !s.matches(reg)
                    && !s.toLowerCase().contains(ExtnKeyConsts.CHILD_TABLE_PREFIX)).collect(Collectors.toList());
            ...
            if (columnData != null && ObjectUtil.isNotEmpty(columnData.getType()) && columnData.getType() == 3) {
                ...
                if (!contains) {
                    if (groupField.startsWith("extn_")) {
            ...
            childFieldList = columnFieldList.stream().filter(s -> s.matches(reg)).map(child -> {
                ...
                String s1 = child.substring(child.lastIndexOf("extn_")).replace("extn_", "");
                String s2 = child.substring(child.indexOf("_") + 1, child.lastIndexOf("_extn"));
    ...
    public static void getConditionSql(QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder form, String databaseProductName, List<VisualColumnSearchVO> searchVOList,
        ...
        try {
            ...
            for (int k = 0; k < searchVOList.size(); k++) {
                ...
                String extnKey = "extnkey";
                if (Objects.nonNull(vo.getConfig())) {
                    extnKey = vo.getConfig().getExtnKey();
                }
                ...
                if (isMultiple) {
                    ...
                    boolean isComSelect = ExtnKeyConsts.COMSELECT.equals(extnKey);
                    if (isComSelect) {
                    ...
                    boolean isCurOrg = ExtnKeyConsts.CURRORGANIZE.equals(extnKey);
                    if (isCurOrg) {
                ...
                if (isMultiple || ExtnKeyConsts.CHECKBOX.equals(extnKey) || ExtnKeyConsts.CASCADER.equals(extnKey)
                        || ExtnKeyConsts.CURRORGANIZE.equals(extnKey) || ExtnKeyConsts.ADDRESS.equals(extnKey) || ExtnKeyConsts.CUSTOMUSERSELECT.equals(extnKey)
                ) {
                    searchType = "2";
                }
                if (ExtnKeyConsts.COM_INPUT.equals(extnKey) || ExtnKeyConsts.TEXTAREA.equals(extnKey)) {
                ...
                if ("1".equals(searchType)) {
                    ...
                    if (ExtnKeyConsts.CUSTOMUSERSELECT.equals(extnKey)) {
                        // 分组 组织 岗位 角色 用户
                    ...
                    switch (extnKey) {
                        ...
                        case ExtnKeyConsts.DATE:
                            ...
                            //处理创建和修改时间查询条件范围
                            if (ExtnKeyConsts.CREATETIME.equals(extnKey) || ExtnKeyConsts.MODIFYTIME.equals(extnKey)) {
    ...
    public static void getSuperSql(QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder where, List<ConditionJsonModel> superQueryList, String matchLogic,
        ...
        superQueryList.stream().filter(condition -> condition.getField().matches(reg)).forEach(su -> {
                    su.setTableName(su.getField().substring(su.getField().indexOf("_") + 1, su.getField().lastIndexOf("_extn")));
                    su.setField(su.getField().substring(su.getField().lastIndexOf("extn_")).replace("extn_", ""));
                }
        );
        ...
        for (int i = 0; i < superQueryList.size(); i++) {
            ...
            String extnKey = conditionJsonModel.getExtnKey();
            BindableColumn<Object> sqlColumn = sqlTable.column(conditionJsonModel.getField());

            if (extnKey.equals(ExtnKeyConsts.DATE) || extnKey.equals(ExtnKeyConsts.CREATETIME) || extnKey.equals(ExtnKeyConsts.MODIFYTIME)) {
            ...
            } else if (extnKey.equals(ExtnKeyConsts.CURRORGANIZE)) {
                if (fieldValue != null) {
...
//            }
    ...
    public static List<BasicColumn> getBasicColumns(List<String> mainTableFields, List<OnlineDynamicSqlModel> sqlModels, OnlineDynamicSqlModel dycModel,
        ...
        List<BasicColumn> mainSqlColumns = mainTableFields.stream().map(m -> {
            ...
            if (isClobDbType && columnListField != null) {
                String extnKey = columnListField.getExtnKey();
                if (extnKey.equals(ExtnKeyConsts.UPLOADFZ) || extnKey.equals(ExtnKeyConsts.UPLOADIMG) || extnKey.equals(ExtnKeyConsts.EDITOR)) {
    ...
    private static List<BasicColumn> getBasicColumnsChild(List<ColumnListField> modelList, OnlineDynamicSqlModel dycModel,
        ...
        List<BasicColumn> mastSqlCols = onlineColumnFieldModels.stream().map(m -> {
            ...
            if (isClobDbType && columnListField != null) {
                String extnKey = columnListField.getConfig().getExtnKey();
                if (extnKey.equals(ExtnKeyConsts.UPLOADFZ) || extnKey.equals(ExtnKeyConsts.UPLOADIMG) || extnKey.equals(ExtnKeyConsts.EDITOR)) {
