...
@RestController
@RequestMapping("/visualdev/OnlineDev" )
public class VisualdevModelDataController extends SuperController<VisualdevModelDataService, VisualdevModelDataEntity> {
    ...
    public ActionResult update(@PathVariable("id" ) String id, @PathVariable("modelId" ) String modelId, @RequestBody VisualdevModelDataUpForm visualdevModelDataUpForm) throws DataException, SQLException, WorkFlowException {
        ...
        if (inlineEdit) {
            ...
            list = list.stream().filter(f -> !f.getVModel().toLowerCase().contains(ExtnKeyConsts.CHILD_TABLE_PREFIX )).collect(Collectors.toList());
        }
    ...
    public ActionResult<DownloadVO> templateDownload(@PathVariable("modelId" ) String modelId) {
        ...
        //子表
        List<FieLdsModel> childFields = allFieLds.stream().filter(f -> f.getConfig().getExtnKey().equals(ExtnKeyConsts.CHILD_TABLE)).collect(Collectors.toList());
        ...
        for (String s : selectKey.stream().filter(s -> !s.toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX )).collect(Collectors.toList())) {
    ...
    public ActionResult<Object> Uploader() {
        ...
        if (file.getOriginalFilename().endsWith(".xlsx" ) || file.getOriginalFilename().endsWith(".xls" )) {
            ...
            //上传文件
            FileInfo fileInfo = FileUploadUtils2.uploadFile(file, filePath, fileName);
    ...
    public ActionResult<Map<String, Object>> ImportPreview(@PathVariable("modelId" ) String modelId, String fileName) throws Exception {
        ...
        try {
            ...
            //子表tableField
            Set<String> tablefield1 = selectKey.stream().filter(s -> s.toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX )).map(s -> s.substring(0, s.indexOf("-" ))).collect(Collectors.toSet());

            String filePath = FileUploadUtils2.getLocalBasePath() + configValueUtil.getTemporaryFilePath();
            FileUploadUtils2.downLocal(configValueUtil.getTemporaryFilePath(), filePath, fileName);
            File temporary = new File(XSSEscape.escapePath(filePath + fileName));
            //判断有无子表
            String tablefield = selectKey.stream().filter(s -> s.toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX )).findFirst().orElse(null);
            //有子表需要取第二行的表头
            ...
            selectKey.stream().forEach(s -> {
                ...
                if (s.toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX )) {
...
//			Map<Integer, Map<String, List<Map<String, Object>>>> IndexMap = new TreeMap<>();
            ...
            for (int t = 0; t < allDataList.size(); t++) {
                ...
                //首条数据不合并
                if (t > 0) {
                    List<Map.Entry<String, Object>> tablefield2 = dataMap.entrySet().stream().filter(e -> !e.getKey().toLowerCase().startsWith(ExtnKeyConsts.CHILD_TABLE_PREFIX)).collect(Collectors.toList());
                    //如果除子表外都为空 则需要合并
