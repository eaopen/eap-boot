...
public class FlowJsonUtil {
    ...
    private static boolean nodeConditionDecide(FlowConditionModel conditionModel, List<ProperCond> conditionList) {
        ...
        for (int i = 0; i < conditionList.size(); i++) {
            ...
            String extnKey = properCond.getExtnKey();
            int fieldType = properCond.getFieldType();
            Object form = fieldType == 1 ? formValue(conditionModel, extnKey, map.get(field)) : formula(properCond, map);
            String formValue = "'" + form + "'";
            List<String> numList = new ArrayList(){{add(ExtnKeyConsts.CALCULATE);add(ExtnKeyConsts.NUM_INPUT);}};
            List<String> timeList = new ArrayList(){{add(ExtnKeyConsts.TIME);}};
            if(numList.contains(extnKey)){
                formValue = "parseFloat("+form+")";
            }
            if(timeList.contains(extnKey)){
...
//                    formValue = "parseFloat(" + form + ")";
            ...
            String valueExtnKey = properCond.getFieldValueExtnKey();
            Object value = fieldValueType == 2 ? filedValue(conditionModel, properCond.getFieldValue(), valueExtnKey, form) : filedData(conditionModel, properCond.getFieldValue(), valueExtnKey, form);
            String fieldValue = "'" + value + "'";
            if(numList.contains(valueExtnKey)){
...
//                    fieldValue = "parseFloat(" + value + ")";
     ...
     * @param value
     */
    private static Object filedValue(FlowConditionModel conditionModel, Object value, String extnKey, Object form) {
        ...
        try {
            ...
            if (value instanceof String[][]) {
                ...
                if (ExtnKeyConsts.CURRORGANIZE.equals(extnKey)) {
                    value = data.stream().filter(t -> t.equals(form)).findFirst().orElse(null);
                } else {
                    value = String.join(",", data);
                }
            } else {
                if (ExtnKeyConsts.CREATETIME.equals(extnKey)) {
                    value = flowTaskEntity.getCreatorTime().getTime() + "";
                } else if (ExtnKeyConsts.CREATEUSER.equals(extnKey)) {
                    value = flowTaskEntity.getCreatorUserId();
                } else if (ExtnKeyConsts.CURRORGANIZE.equals(extnKey)) {
                    value = userEntity.getOrganizeId();
                } else if (ExtnKeyConsts.CURRPOSITION.equals(extnKey)) {
                    value = userEntity.getPositionId();
                } else if (ExtnKeyConsts.MODIFYTIME.equals(extnKey)) {
                    value = flowTaskEntity.getLastModifyTime().getTime() + "";
                } else if (ExtnKeyConsts.MODIFYUSER.equals(extnKey)) {
     ...
     * @param value
     */
    private static Object filedData(FlowConditionModel conditionModel, Object value, String extnKey, Object form) {
     ...
     * @param form
     */
    private static Object formValue(FlowConditionModel conditionModel, String extnKey, Object form) {
        Object result = form;
        UserEntity userEntity = conditionModel.getUserEntity();
        FlowTaskEntity flowTaskEntity = conditionModel.getFlowTaskEntity();
        try {
