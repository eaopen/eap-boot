...
public class FlowFormDataUtil {
    ...
    private Map<String, Object> tableData(DataModel dataModel, List<FormAllModel> formAllModel) throws WorkFlowException {
        ...
        try {
            ...
            List<FormAllModel> mastForm = formAllModel.stream().filter(t -> FormEnum.mast.getMessage().equals(t.getExtnKey())).collect(Collectors.toList());
            ...
            //子表数据
            List<FormAllModel> tableForm = formAllModel.stream().filter(t -> FormEnum.table.getMessage().equals(t.getExtnKey())).collect(Collectors.toList());
            ...
            //副表
            Map<String, List<FormAllModel>> mastTableAll = formAllModel.stream().filter(t -> FormEnum.mastTable.getMessage().equals(t.getExtnKey())).collect(Collectors.groupingBy(e -> e.getFormMastTableModel().getTable()));
    ...
    private Object info(FieLdsModel fieLdsModel, Object dataValue, boolean isTable) {
        ...
        String extnKey = fieLdsModel.getConfig().getExtnKey();
        ...
        switch (extnKey) {
            ...
            case ExtnKeyConsts.MODIFYUSER:
                if (ObjectUtil.isNotEmpty(value)) {
                    ...
                    value = ExtnKeyConsts.CREATEUSER.equals(extnKey) ? " " : null;
                }
                break;
            ...
            case ExtnKeyConsts.UPLOADIMG:
                ...
                break;
            case ExtnKeyConsts.CHECKBOX:
            case ExtnKeyConsts.DATERANGE:
            case ExtnKeyConsts.TIMERANGE:
                if (value == null) {
                    value = new ArrayList<>();
                } else {
                    if (isTable) {
            ...
            case ExtnKeyConsts.ADDRESS:
                ...
                break;
            case ExtnKeyConsts.SELECT:
            case ExtnKeyConsts.USERSELECT:
            case ExtnKeyConsts.CUSTOMUSERSELECT:
            case ExtnKeyConsts.DEPSELECT:
            case ExtnKeyConsts.POSSELECT:
            case ExtnKeyConsts.TREESELECT:
            case ExtnKeyConsts.ROLESELECT:
            case ExtnKeyConsts.GROUPSELECT:
                if (isTable) {
                    if (multiple) {
                        value = JsonUtil.getJsonToList(String.valueOf(value), String.class);
                    }
                ...
                break;
            case ExtnKeyConsts.POPUPTABLESELECT:
                if (isTable) {
                    try {
                        value = JsonUtil.getJsonToList(String.valueOf(value), String.class);
                    } catch (Exception e) {
...
//			addField(tableModelList, dataModel.getLink());
        ...
        for (String key : childMap.keySet()) {
            ...
            for (Map<String, Object> objectMap : chidList) {
                ...
                for (String childKey : columMap.keySet()) {
                    ...
                    getDSL(isOracle, sqlTable, null,generalInsertDSL, fieLdsModel.getFormat(), extnkey, childKey, data);
                }
    ...
    private void createMastTable(List<FormAllModel> formAllModel, DataModel dataModel, Connection conn, Map<String, Object> result) throws SQLException {
        ...
        for (String key : mastTableAll.keySet()) {
            ...
            for (FormAllModel model : masTableList) {
                ...
                getDSL(isOracle, sqlTable, null,generalInsertDSL, fieLdsModel.getFormat(), extnkey, field, data);
            }
            //sql主键
            if (primaryKeyPolicy == 1) {
                generalInsertDSL = generalInsertDSL.set(sqlTable.column(childKeyName)).toValue(RandomUtil.uuId());
    ...
    private void createMast(List<FormAllModel> formAllModel, DataModel dataModel, Connection conn, Map<String, Object> result) throws SQLException {
        ...
        List<FormAllModel> mastForm = formAllModel.stream().filter(t -> FormEnum.mast.getMessage().equals(t.getExtnKey())).filter(t -> StringUtil.isNotEmpty(t.getFormColumnModel().getFieLdsModel().getVModel())).collect(Collectors.toList());
        //新增字段
        List<String> filedModel = new ArrayList<>();
    ...
    private Object create(FieLdsModel fieLdsModel, Object dataValue, boolean isTable, UserEntity userEntity, boolean isLink) {
        ...
        //外链跳过系统参数生成
        if (isLink) {
            value = linkDefaultValue(dataValue, isTable, extnKey, value);
            return value;
        }
        switch (extnKey) {
            case ExtnKeyConsts.CREATEUSER:
            ...
            case ExtnKeyConsts.DATE:
                ...
                break;
            case ExtnKeyConsts.NUM_INPUT:
            case ExtnKeyConsts.CALCULATE:
                if (isTable) {
                    try {
     ...
     * @return
     */
    private Object linkDefaultValue(Object dataValue, boolean isTable, String extnKey, Object value) {
        switch (extnKey) {
            case ExtnKeyConsts.DATE:
                if (isTable) {
                    try {
...
//            flowFormDataMapper.delete(deleteRender);
            ...
            for (Map<String, Object> objectMap : chidList) {
                ...
                for (String childKey : columMap.keySet()) {
                    ...
                    String extnkey = fieLdsModel.getConfig().getExtnKey();
                    Object data = objectMap.get(childKey);
                    if (ExtnKeyConsts.CURRORGANIZE.equals(extnkey) || ExtnKeyConsts.CURRPOSITION.equals(extnkey) || ExtnKeyConsts.CREATETIME.equals(extnkey) || ExtnKeyConsts.CREATEUSER.equals(extnkey)) {
                        continue;
                    }
                    //处理系统自动生成
                    data = this.update(fieLdsModel, data, true);
                    getDSL(isOracle, sqlTable, null,generalInsertDSL, fieLdsModel.getFormat(), extnkey, childKey, data);
                    getDSL(isOracle, sqlTable, updateModelUpdateDSL,null, fieLdsModel.getFormat(), extnkey, childKey, data);
                }
    ...
    private void updateMastTable(List<FormAllModel> formAllModel, DataModel dataModel, Connection conn, Map<String, Object> result) throws SQLException {
        ...
        for (String key : mastTableAll.keySet()) {
            ...
            for (FormAllModel model : masTableList) {
                ...
                //处理系统自动生成
                data = this.update(fieLdsModel, data, true);
                getDSL(isOracle, sqlTable, updateModelUpdateDSL,null, fieLdsModel.getFormat(), extnkey, field, data);
            }
            //sql主键
            if (primaryKeyPolicy == 1) {
    ...
    private void getDSL(Boolean isOracle, SqlTable sqlTable, UpdateDSL<UpdateModel> updateModelUpdateDSL, GeneralInsertDSL generalInsertDSL,
                        String format, String extnkey, String field, Object data) {
        ...
        if (data == null || StringUtil.isEmpty(data.toString())) {
            ...
            if (isOracle && (ExtnKeyConsts.DATE.equals(extnkey) || ExtnKeyConsts.MODIFYTIME.equals(extnkey) || ExtnKeyConsts.CREATETIME.equals(extnkey))) {
