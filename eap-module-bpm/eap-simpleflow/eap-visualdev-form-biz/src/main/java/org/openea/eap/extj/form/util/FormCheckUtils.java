...
public class FormCheckUtils {
        ...
        try {
            ...
            for (FieLdsModel fieLdsModel : mainFields) {
                ...
                if (ObjectUtil.isNotNull(o) && StringUtil.isNotEmpty(o.toString())) {
                    ...
                    if(fieldName.toLowerCase().contains("_extn_")){//附表字段名称
                        fieldName= fieldName.split("_extn_")[1];
                    }
            ...
            List<FieLdsModel> childFieldList = fields.stream().filter(f -> ExtnKeyConsts.CHILD_TABLE.equals(f.getConfig().getExtnKey())).collect(Collectors.toList());
    ...
    private static List<FieLdsModel> checkInputUnique(List<FieLdsModel> fields) {
        List<FieLdsModel> inputUnique = fields.stream().filter(field -> ExtnKeyConsts.COM_INPUT.equals(field.getConfig().getExtnKey())
