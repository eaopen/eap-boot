...
public class VisualDevTableCre {
    ...
    private void fieldsTableName(JSONArray jsonArray, List<TableModel> tableModels) {
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            String extnkey = jsonObject.getJSONObject("__config__").getString("extnKey");
            ...
            if (childrenListAll.contains(extnkey) || StringUtil.isEmpty(extnkey)) {
    ...
    private TableModel fieldsModel(JSONObject jsonObject, List<TableModel> tableModels) {
        ...
        if (FormEnum.relationFormAttr.getMessage().equals(extnkey) || FormEnum.popupAttr.getMessage().equals(extnkey)) {
            if (StringUtil.isNotEmpty(relationField)) {
                ...
                String model = relationField.split("_extnTable_" )[0];
                jsonObject.put("relationField" , model + "_extnTable_" + tableName.getTable() + (isSubTable ? "0" : "1" ));
            }
        }
        return childTableaa;
    }
    ...
    public List<TableModel> tableList(VisualTableModel visualTableModel) throws WorkFlowException {
        ...
        try {
            ...
            for (FormAllModel model : formAllModel) {
                if (FormEnum.mast.getMessage().equals(model.getExtnKey())) {
                    FieLdsModel fieLdsModel = model.getFormColumnModel().getFieLdsModel();
                    this.fieldList(fieLdsModel, table, fieldList);
                } else if (FormEnum.table.getMessage().equals(model.getExtnKey())) {
        ...
        if (StringUtil.isNotEmpty(vmodel)) {
            ...
            if (StringUtil.isNotEmpty(fieLdsModel.getVModel())) {
                List<String> text = new ArrayList() {{
                    add(ExtnKeyConsts.UPLOADIMG);
                    add(ExtnKeyConsts.UPLOADFZ);
                }};
                if (text.contains(extnkey)) {
                    fieldForm.setDataType("text" );
                }
                if (extnkey.equals(ExtnKeyConsts.EDITOR)) {
                    fieldForm.setDataType("text" );
                }
                List<String> date = new ArrayList() {{
                    add(ExtnKeyConsts.MODIFYTIME);
                    add(ExtnKeyConsts.CREATETIME);
                    add(ExtnKeyConsts.DATE);
                }};
                if (date.contains(extnkey)) {
                    fieldForm.setDataType("datetime" );
                }
                if (ExtnKeyConsts.NUM_INPUT.equals(extnkey)) {
