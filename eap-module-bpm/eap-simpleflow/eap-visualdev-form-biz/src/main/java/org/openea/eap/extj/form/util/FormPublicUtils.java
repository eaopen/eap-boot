...
public class FormPublicUtils {
    ...
    public static void removeUseless(List<FieLdsModel> fieldsModelList) {
        for (int i = 0; i < fieldsModelList.size(); i++) {
            if (fieldsModelList.get(i).getConfig().getExtnKey() == null) {
                continue;
            }
            if (fieldsModelList.get(i).getConfig().getExtnKey().equals(ExtnKeyConsts.CHILD_TABLE)) {
                continue;
            }
        }
    }
    ...
    public static List<FieLdsModel> recursionFields(List<TableModel> tableModels, List<FieLdsModel> fieLdsModelList) {
        ...
        childTable.stream().forEach(formModel -> {
            String extnKey = formModel.getExtnKey();
            ...
            configModel.setExtnKey(extnKey);
    ...
    public static Map<String, Object> getDataMap(List<Map<String, Object>> dataList, FieLdsModel swapModel) {
        String label;
        String value;
        if (swapModel.getConfig().getExtnKey().equals(ExtnKeyConsts.CASCADER) || swapModel.getConfig().getExtnKey().equals(ExtnKeyConsts.TREESELECT)) {
    ...
    public static void recurseFiled(List<FieLdsModel> modelList, List<FieLdsModel> mainFields, List<FieLdsModel> childFields, List<FormModel> models) {
        for (FieLdsModel fieLdsModel : modelList) {
            ...
            boolean isExtnKey = StringUtil.isEmpty(extnkey);
            ...
            } else if (FormEnum.groupTitle.getMessage().equals(extnkey) || FormEnum.divider.getMessage().equals(extnkey) || FormEnum.ExtnText.getMessage().equals(extnkey)) {

            } else if (FormEnum.QR_CODE.getMessage().equals(extnkey) || FormEnum.BARCODE.getMessage().equals(extnkey)) {
    ...
    public static void recursionFieldsExceptChild(List<FieLdsModel> allFields, List<FieLdsModel> fieLdsModelList) {
        for (FieLdsModel fieLdsModel : fieLdsModelList) {
            ConfigModel config = fieLdsModel.getConfig();
            String extnKey = config.getExtnKey();
            if (ExtnKeyConsts.CHILD_TABLE.equals(extnKey)) {
                allFields.add(fieLdsModel);
                continue;
            } else {
     ...
     * @return
     */
    public static boolean dateTimeCondition(FieLdsModel swapDataVo, String format, Object value, Map<String, Object> data, String extnKey) {
        long valueTimeLong;
        //输入值转long
        ...
        if ((swapDataVo.getConfig().getStartTimeRule() && StringUtil.isNotEmpty(swapDataVo.getConfig().getStartTimeValue()))
            ...
            //根据类型获取开始时间戳
            long startTimeLong = getDateTimeLong(data, extnKey, startTimeValue, startTimeType, startTimeTarget, startTimeRelationField);
        ...
        if ((swapDataVo.getConfig().getEndTimeRule() && StringUtil.isNotEmpty(swapDataVo.getConfig().getEndTimeValue()))
            ...
            //根据类型获取开始时间戳
            long endTimeLong = getDateTimeLong(data, extnKey, endTimeValue, endTimeType, endTimeTarget, endTimeRelationField);
     ...
     * @return
     */
    private static long getDateTimeLong(Map<String, Object> data, String extnKey, String timeValue, String timeType, String timeTarget,
        ...
        switch (timeType) {
            ...
            case "2"://表单字段
                if (stringimeRelationField != null) {
                    String fieldValue = "";
                    if (stringimeRelationField.toLowerCase().contains(ExtnKeyConsts.CHILD_TABLE_PREFIX)) {//子
            ...
            case "4"://当前时间前
                ...
                if (ExtnKeyConsts.DATE.equals(extnKey)) {
                    switch (timeTarget) {
                        case "1"://年
