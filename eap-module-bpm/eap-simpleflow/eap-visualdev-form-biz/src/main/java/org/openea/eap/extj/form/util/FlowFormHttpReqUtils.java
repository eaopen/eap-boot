...
public class FlowFormHttpReqUtils {
    ...
    private static ExtnOauthConfig extnOauthConfig;

    @Autowired
    public void setExtnOauthConfig(ExtnOauthConfig extnOauthConfig) {
        FlowFormHttpReqUtils.extnOauthConfig = extnOauthConfig;
    }
    ...
    private String getReqURL(FlowFormEntity flowFormEntity, String id) {
        ...
        if (!isHttp) {
            //补全(内部)
            requestURL = extnOauthConfig.getExtnDomain() + requestURL;
        }
