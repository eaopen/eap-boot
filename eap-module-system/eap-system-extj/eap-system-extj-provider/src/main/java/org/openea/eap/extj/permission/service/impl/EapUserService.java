...
import org.openea.eap.extj.permission.mapper.ExtjUserMapper;
...
@Service
@Slf4j
public class EapUserService extends SuperServiceImpl<ExtjUserMapper,UserEntity> implements UserService {
    ...
    public List<UserEntity> getList(Pagination pagination, Boolean filterCurrentUser) {
        PageResult<AdminUserDO> userList = adminUserService.getUserList(getPageParam(pagination), pagination.getKeyword(), filterCurrentUser);
    ...
    private PageParam getPageParam(Pagination pagination){
        if(pagination==null){
            return null;
        }
     ...
     */
    @Override
    public ExtjUserMapper getBaseMapper() {
        return null;
    }
