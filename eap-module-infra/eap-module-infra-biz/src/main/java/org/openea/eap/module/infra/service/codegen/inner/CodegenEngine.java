package org.openea.eap.module.infra.service.codegen.inner;
...
import org.openea.eap.module.infra.enums.codegen.CodegenVOTypeEnum;
import org.openea.eap.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 eap-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 eap-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("org.openea.eap.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/eap-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/eap-module-${table.moduleName}-server", "");
    ...
    private static String mapperXmlFilePath() {
        return "eap-module-${table.moduleName}/" + // 顶级模块
                "eap-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "eap-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
